    ...
    *,
    ...
    for ep in iterator:
        ...
        while True:
            ...
            # on `terminated`.
            # Treat max_steps truncation as termination to stop bootstrapping
            if max_steps_per_episode is not None and ep_length + 1 >= max_steps_per_episode:
                terminated = True

            done = terminated or truncated
            next_action = (
                None if terminated else agent.select_action(next_obs)
            )
        ...
        if hasattr(agent, "W"):
            history.weight_norms.append(float(np.linalg.norm(agent.W)))
